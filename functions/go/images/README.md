...
```
./scripts/sync-images.sh go
```

che copia da `functions/images/`, la sorgente unica (vedi il README lì per il perché e per i vincoli sui file). Va lanciato **prima di `go build` o di `func start`** in locale, ed è lo stesso comando che userà la pipeline CI prima del deploy.
