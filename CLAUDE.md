...
- **Mai aggiungere a `TODO.md` un task solo per spuntarlo.** `TODO.md` traccia il lavoro **da fare**: una voce si aggiunge quando il lavoro è previsto, non a cose fatte per lasciarne traccia. Quello che è stato fatto, deciso o scoperto va **solo** nel decision log — è lì che sta la storia del progetto. Un `TODO.md` pieno di `[x]` aggiunti a posteriori smette di essere un tracker e diventa un secondo log, peggiore dell'originale perché senza motivazioni e senza numerazione.
