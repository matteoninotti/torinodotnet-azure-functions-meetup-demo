...
- [ ] **[M→S retrocesso] Controllare la quota regionale di core effettiva.** ⚠️ **Scoperta (D29)**: non esiste un comando `az` documentato per leggere la quota *corrente* prima che esista un'app. L'unico strumento ufficiale è il **Flex Consumption Quota tool nel portale**, e richiede di aprire un'app Flex già esistente ("Diagnose and solve problems" → cerca "Flex Consumption Quota"). Rimandato alla Fase 2, subito dopo il primo deploy Python — non prima, perché non è verificabile prima.
