...
- [ ] **[M] Rimettere in piedi il worker Python, rotto dal primo deploy .NET** (D78). I due worker puntavano allo stesso container di deploy e su Flex il pacchetto è un solo blob per container: il deploy .NET ha sovrascritto quello Python, che ora risponde `404` su ogni route. Bicep corretto (un container per linguaggio) e da rideployare, poi `workflow_dispatch` con `language: python` **prima** di quello .NET.
