...
- [ ] **[M] RPS target provvisorio** (D43). **Metrica 1: 10 req/s, misurato** — 301 richieste, 0 fallimenti, nessuna `dropped_iteration`, ~13 istanze contro un tetto di 200 (D54). **Metrica 3: ancora da fissare**, va misurato con un burst da zero istanze, quindi dipende dal task dello scale-to-zero qui sotto. Il tetto di Metrica 3 è `maximumInstanceCount`, non l'RPS che si chiede a k6 — con concorrenza 1 il numero di istanze *è* il numero di richieste in volo.
...
- [ ] **[M] Misurare il tempo di scale-to-zero** osservando `InstanceCount`, e scriverlo nel log (D16). ⚠️ `InstanceCount` va interrogata con **aggregazione `Count`, non `Maximum`** (D55): ogni istanza emette un campione di valore 1 ogni 30 s, quindi con `Maximum` il risultato è sempre `1.0` e sembra che l'app non scali mai. Serve un protocollo pulito: app a zero verificata, carico noto, stop netto, poi polling fino alla scomparsa dei campioni.
- [ ] **[S] Container Apps Job con k6** per i run finali: `replicaRetryLimit` a 0, `replicaTimeout` dimensionato. ⚠️ La promozione a `[M]` decisa in D48 è stata **annullata da D53**: si basava su un numero sbagliato (i ~765 ms erano cold start, non rete — il RTT reale è ~34 ms). Resta `[S]`. **Prima di ridecidere va misurato** se il Mac regge il tasso della Metrica 3 senza diventare lui il collo di bottiglia.
