...
- [ ] **[M] Istanziare il modulo Bicep per .NET**: piano Flex dedicato + function app, `dotnet-isolated` versione `'10'`, stessa instance size e stessa regione. Piano dedicato perché su Flex vale una sola app per piano. ⏳ **Scritto e verificato con `what-if`** (`2 to create`, concorrenza di Python invariata), ma `az deployment group create` è bloccato dal classificatore dell'harness: **lo lancia Matteo** (D72).
- [ ] **[M] Deploy del worker .NET e prima chiamata riuscita da internet**, verificata in modo indipendente dal workflow come si è fatto per Python (D41). Parte da `main`, quindi dopo il merge di `phase-4` (D44). È il passo che dimostra anche la concorrenza, la pipeline e la telemetria qui sotto: finché non è verde, sono scritte ma non dimostrate.
- [ ] **[M] Concorrenza a 1** su questa app (D22). Dichiarata nel Bicep (D39): da **rileggere dalla risorsa** dopo il deploy con `az resource show --resource-type Microsoft.Web/sites --api-version 2024-04-01`, perché `az functionapp show` restituisce `null` su tutto `functionAppConfig`.
- [ ] **[M] Percorso di build .NET nella pipeline** (D63, D73). ⏳ **Scritta**: `setup-dotnet`, `dotnet test` e `dotnet publish` condizionati al linguaggio, `package:` e `remote-build:` scelti da uno step, e un controllo che l'output di publish contenga davvero le immagini. Resta `[ ]` finché un deploy vero non la esercita.
